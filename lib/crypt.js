...
class Crypt {
  ...
  static stringToArrayBufferInUtf8(str) {
    ...
    const new_TextEncoder = typeof window === 'undefined' ? TextEncoder : window.TextEncoder;
    // always utf-8
    let encoder = new new_TextEncoder();
  ...
  static utf8ArrayBufferToString(strBuffer) {
    // if not browser env, then require node.js's util. otherwise just use window's
    const new_TextDecoder = typeof window === 'undefined' ? TextEncoder : window.TextDecoder;
    let decoder = new new_TextDecoder('utf-8');
